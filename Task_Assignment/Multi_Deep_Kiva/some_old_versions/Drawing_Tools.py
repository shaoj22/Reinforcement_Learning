...
class Draw_Opt():
    def __init__(self, problem):
        ...
        self.st = problem.st  # ABC货架

    def _draw_t(self, ax, p, s, h, e, g, st):
        for i in range(self.N):
            ...
            if g[i] == 1: # 目标货架
                ...
                if st[i] == 1 :
                    self._draw_pixel(ax, row, col, facecolor='gray')
                if st[i] == 2:
                    self._draw_pixel(ax, row, col, facecolor='gray')
                if st[i] == 3:
                    self._draw_pixel(ax, row, col, facecolor='gray')
    ...
    def show_map(self):
        ax = plt.subplot(111)
        self._draw_t(ax, self.p, self.s, self.h0, self.e0, self.g0, self.st)
        plt.show()
