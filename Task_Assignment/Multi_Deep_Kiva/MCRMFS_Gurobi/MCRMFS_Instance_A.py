...
class Instance_all():
    def __init__(self, w_num=2, l_num=2, m=3, n=4, R=4, num_g=4, percent_e=0.5, seed=0):
        """instance of kiva
    ...
    def init_s(self):
        ...
        for i in range(self.W):
            for j in range(self.L):
                if i % (self.m + 1 ) == 1 or j % (self.n + 1) == 1 or i == 0 or j == 0 or i == self.col_num - 1 or j == self.row_num - 1:
                    S[i][j] = 1
        # 转换
        s = self.tf(S)
    ...
    def init_A(self):
        ...
        for i in range(self.W):
            for j in range(self.L):
                # 选出货架区域
                if i not in odd_row and i not in even_row and j not in odd_col and j not in even_col and i != 0 and i != (self.W-1) and j != 0 and j != (self.L-1):
                    # 到右边
