...
class PGPolicy(nn.Module):
    ...
    def forward(self, state, info, exploit=0):
        ...
        info["rack_candidates"] = rack_candidates
        if len(rack_candidates) == 0: # 无可选货架，返回None
            return None
        elif len(rack_candidates) == 1: # 只有一个选项，直接选
            rack = rack_candidates[0]
        else:
            # network calculation
            # q = env.robots[env.main_robot_id].pos
            # info["q1"] = q
            # state_v = torch.FloatTensor(state)[rack_candidates]
            # state_q = torch.FloatTensor(state)[q]
            # logits1 = self.net(state=state_v, state_q=state_q)
            # # choose rack
            # prob1 = torch.softmax(logits1, dim=0).detach().numpy()
            # # assert prob1[0] is not None, "invalid logits"
            # if exploit == 0:
            #     ri = np.random.choice(len(prob1), p=prob1)
            # else:
            #     ri = np.argmax(prob1)
            # 启发式引导策略
            ri = -1
            min_d = np.inf
            robot_pos = env.robots[env.main_robot_id].pos
            robot_corr = [robot_pos // self.col_num, robot_pos % self.col_num]
            for i in range(len(rack_candidates)):
                rack_corr = [rack_candidates[i] // self.col_num, rack_candidates[i] % self.col_num]
                dist = abs(robot_corr[0]-rack_corr[0]) + abs(robot_corr[1]-rack_corr[1])
                if dist < min_d:
                    min_d = dist
                    ri = i
            rack = rack_candidates[ri]
    ...
    def update(self, batch):
        ...
        for episode_i in range(len(batch_states)):
            for step_i in range(len(batch_states[episode_i])):
                ...
                # get masks
                rack_candidates = info["rack_candidates"]
                # place_mask = info["place_mask"]
                ...
                # calculate log_prob
                if len(rack_candidates) == 1:
                    continue
                state_v = torch.FloatTensor(state)[rack_candidates]
                state_q = torch.FloatTensor(state)[info["q1"]]
                logits1 = self.net(state=state_v, state_q=state_q)
                ...
                # calculate loss
                loss = -log_prob1[rack_candidates.index(action[0])] * reward #- log_prob2[action[1]] * reward #!check
                loss.backward()
...
class HeuristicPolicy(nn.Module):
    ...
    def forward(self, state, info, exploit=0):
        ...
        if len(rack_candidates) == 0: # 无可选，返回None
            return None
        # 启发式选择随机一个阻碍货架
        ri = np.random.randint(len(rack_candidates))
        # ri = 0
        rack = rack_candidates[ri] 
